...
  function parseReply(data) {
    // Give navigation replies priority
    if (data.navigationPath && data.reply) {
        return data.reply; // e.g., "Sure, opening..."
    }
...
      ]);
      ...
      if (data.navigationPath) {
        ...
        // Use a timeout so the user can read the "Navigating..." message
        setTimeout(() => {
          setIsOpen(false); // Close the chat
          router.push(data.navigationPath); // Redirect
        }, 1000); // 1-second delay
      }
      // ** END OF NEW LOGIC **
...
    if (!input.trim() || loading) return;
    ...
    if (msg.startsWith("open ") || msg.startsWith("go to ") || msg.startsWith("navigate to ")) {
        sendMessage({ messageText: input.trim(), actionType: null });
    } else {
        // Default behavior (could be 'enhance' or 'write' based on context)
        // For simplicity, we'll just send it as a general message.
        // You could make this 'enhance' by default if input is present.
        sendMessage({ messageText: input.trim(), actionType: null });
    }
  };
